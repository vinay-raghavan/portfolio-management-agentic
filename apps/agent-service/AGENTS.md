...
return codes, generated artifact file names, and next actions. The triage
command writes `artifacts/evals/triage-report.json`; use it to classify failed
cases before changing instructions, tool descriptions, or eval cases.
...
- **Provider metadata boundary**: provider profile refreshes and scheduled refresh orchestration may write only sanitized validation metadata, import-job summaries, readiness/backoff state, and normalized configured market/context records; never persist resolved local paths, raw provider payloads, broker tokens, or account data.
