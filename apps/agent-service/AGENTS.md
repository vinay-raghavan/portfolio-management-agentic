...
- **Provider metadata boundary**: provider profile refreshes may write only sanitized validation metadata, import-job summaries, and normalized configured market/context records; never persist resolved local paths, raw provider payloads, broker tokens, or account data.
