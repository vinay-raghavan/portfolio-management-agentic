...
Set `PORTFOLIO_STORAGE_BACKEND=postgres` with `PORTFOLIO_DATABASE_URL` and `PORTFOLIO_TENANT_ID` for production-like container runs; the repository Compose file runs Alembic migrations before MCP starts. Set `PORTFOLIO_STORAGE_BACKEND=sqlite` for local/offline compatibility.
Strategy drafts, backtest requests, paper orders, positions, fills, approvals, audit events, market snapshots, screener runs, universes, factor context, provider configuration profiles, and import-refresh jobs use tenant-scoped Postgres when production-like storage is selected. Set `PAPER_LEDGER_DB_PATH`, `MARKET_DATA_DB_PATH`, and `PROVIDER_CONFIG_DB_PATH` only for explicit SQLite offline fallbacks. Docker or Podman Compose keeps `/data/paper-ledger.db`, `/data/market-data.db`, and `/data/provider-config.db` on the same named volume for those fallback runs.
