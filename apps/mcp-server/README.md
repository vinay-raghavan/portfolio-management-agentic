...
Set `PAPER_LEDGER_DB_PATH` only for explicit SQLite-backed strategy, backtest, and paper-ledger persistence in local/offline compatibility runs. Docker or Podman Compose uses `/data/paper-ledger.db` on a named volume for that fallback.
Market snapshots, screener runs, universes, factor context, provider configuration profiles, and import-refresh jobs use tenant-scoped Postgres when `PORTFOLIO_STORAGE_BACKEND=postgres`, `PORTFOLIO_DATABASE_URL`, and `PORTFOLIO_TENANT_ID` are set. Set `MARKET_DATA_DB_PATH` and `PROVIDER_CONFIG_DB_PATH` only for explicit SQLite offline fallbacks. Docker or Podman Compose keeps `/data/market-data.db` and `/data/provider-config.db` on the same named volume for those fallback runs.
