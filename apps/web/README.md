...
- Shows the operator dashboard plus focused workflow pages for screeners, strategy/backtest review, paper approvals, reports, and provider settings with configured-source setup, required env keys, active adapter modes, setup-gap feedback, configured-file validation, provider profiles, refresh readiness, full-refresh controls, per-provider backoff state, and import-job feedback.
