...
Next, add configured-provider import reconciliation that compares dry-run preview counts, the latest refresh job, and structured cache rows so operators can see whether configured stores match the last expected import.
...
Next test to write: a contract showing configured-provider import reconciliation reports preview counts, latest job counts, and stored row counts for market, universe, and factor stores without exposing local source paths, database paths, raw provider payloads, or credentials.
