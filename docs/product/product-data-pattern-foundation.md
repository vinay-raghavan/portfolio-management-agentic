...
Next, add configured-provider import dry-run previews that show normalized counts, sample identifiers, target stores, and warnings before refresh jobs write local caches.
...
Next test to write: a contract showing configured-provider import dry-run previews return target stores, normalized counts, sample identifiers, and warnings without writing cache rows or exposing local source paths, provider payloads, or credentials.
