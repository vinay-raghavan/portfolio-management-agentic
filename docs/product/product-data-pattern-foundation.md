...
- Use Postgres for production-like users, portfolios, watchlists, strategies, paper orders, simulated fills, approvals, audit logs, market snapshots, screener runs, provider universe/factor context, provider profiles, and provider import-refresh jobs. Keep SQLite as an explicit offline fallback.
- Use structured local storage for market snapshots, provider context snapshots, screener runs, provider profile metadata, provider import-refresh jobs, paper-ledger state, and backtest datasets. The current implementation uses tenant-scoped Postgres for durable strategy drafts, backtest requests, paper orders, positions, fills, approvals, audit events, market snapshots, screener runs, universes, fundamentals, sentiment, volatility, macro context, provider profiles, and provider import-refresh jobs in production-like mode, SQLite for offline paper-ledger/market/provider-context/provider-profile caches, and configured read-only JSON market-snapshot, universe, fundamentals, sentiment, volatility, and macro adapters for local provider exports; DuckDB or Parquet can replace or supplement this when batch analytics volume justifies it.
