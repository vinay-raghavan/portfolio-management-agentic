...
Next, run the credential-gated model eval baseline, review the baseline summary, triage report, traces, and grade artifacts, and use failures to tune agent instructions, tool descriptions, and trajectory checks for paper-trading, provider-readiness, grounding, tool-trajectory, and forbidden-action workflows.
