...
because evidence is fragmented, recommendations are difficult to explain, and
...
TradePilot Sentinel addresses the workflow around the decision. It combines
agent coordination, deterministic analysis, explicit permissions, human
approval, and durable audit state in one system.

## The Solution

TradePilot Sentinel is an agentic trading workflow and risk-control platform.
The Kaggle capstone demonstrates it with offline-safe fixture data, configured
read-only provider adapters, and approval-gated paper simulation.
...
a redacted report. Every stage remains visible and reviewable.

The capstone deliberately stops at paper simulation. That is the safe evidence
path, not the product's permanent boundary. Future execution adapters would
require separate broker isolation, stronger approvals, production
observability, audit review, rollback, and release controls. None of those
future capabilities are represented as implemented today.
...
workflow route, calls bounded tools, and synthesizes the result in plain
language.

The language model does not own trading facts or permissions. A streamable HTTP
MCP server exposes the safe tool catalog. Deterministic domain services own
provider validation, screener rules, factor scoring, recommendations,
...
The model layer is configurable. Gemini is the default capstone provider, while
the adapter design includes Claude, OpenAI-compatible, and Ollama paths. The
MCP and policy boundaries remain the same regardless of provider, so changing
the model cannot expand permissions.
...
reconciliation, stale data, missing strategy evidence, failed risk checks, or
unready backtest state can remove the paper-order next action.
...
authoritative. This is a deliberately bounded retrieval pattern rather than a
vector database controlling decisions.
...
The proposal is persisted as pending approval. A human must explicitly approve
the simulation. Only then can the MCP tool create a simulated paper fill. The
paper ledger updates positions and accounting and emits redacted audit events.
A report joins the readiness snapshot, approval, fill, accounting, risk state,
and audit rows into one reviewable result.
...
create defense in depth outside the model prompt.
...
The credentialed Gemini baseline completed 15 inference cases covering
positive workflows and forbidden-action requests. The evaluation combines an
...
trajectory score also identifies useful future work: tighten routing so the
agent selects the preferred tool sequence more consistently. Three
response-quality judge outputs produced parse errors and are retained as a
known baseline limitation rather than being hidden.
...
TradePilot Sentinel turns a sequence of manual handoffs into a governed,
...
or is not eligible. Operators can inspect provider readiness and reconciliation
before trusting a recommendation. Every consequential simulation leaves an
...
TradePilot Sentinel's central principle is durable: the model can recommend,
