...
| `get_market_data_snapshot` | read-only | Return fixture-backed OHLCV snapshot and metrics for one symbol, with optional local cache persistence. |
...
The product data, provider, pattern, recommendation, report, strategy-history, backtest, and paper-ledger tools do not write live orders, retrieve broker trading tokens, expose provider credential values, or bypass paper-trading approval gates. Read-only market-data tools may persist local cache rows when `MARKET_DATA_DB_PATH` is configured, but those writes are limited to market snapshots and screener-run payloads. Simulated fills are paper-only, require prior approval, and update only the paper ledger. Report tools return JSON-ready data and do not write files.
