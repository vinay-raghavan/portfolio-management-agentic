...
| `get_fyers_connection_health` | read-only | Return sanitized FYERS read-only connector health, fixture/connector state, allowed read surfaces, and reconnect status without credential values. |
| `get_fyers_quote` | read-only | Return one normalized FYERS quote snapshot for an exchange-qualified symbol, failing as unavailable when the connector/source has no quote. |
| `get_fyers_ohlcv_history` | read-only | Return normalized FYERS OHLCV history for an exchange-qualified symbol, failing as unavailable when the connector/source has no history. |
| `get_fyers_depth` | read-only | Return normalized FYERS market depth for an exchange-qualified symbol, failing as unavailable when the connector/source has no depth. |
| `get_fyers_instrument_metadata` | read-only | Return normalized FYERS instrument metadata for an exchange-qualified symbol, failing as unavailable when the connector/source has no metadata. |
| `get_fyers_option_chain` | read-only | Return normalized FYERS option-chain contracts for an exchange-qualified underlying, failing as unavailable when the connector/source has no chain. |
| `get_fyers_account_snapshot` | read-only | Return a normalized FYERS account snapshot with signed holdings/positions, funds, and hashed provider history identifiers while keeping provider data separate from the paper ledger. |
...
| `issue_paper_execution_grant` | approval-required | Execution grants must be issued by protected server-side approval APIs from authenticated human identity, not model output. |
| `evaluate_paper_execution_order` | approval-required | Deterministic paper execution decisions may run only under protected worker/API paths with scoped grants, never as model-visible MCP. |
| `simulate_approved_paper_fill` | approval-required | Paper-ledger mutation belongs to protected human/API or execution-grant paths, not model-visible MCP. |
