...
| `list_provider_import_jobs` | read-only | Return metadata-only provider import-refresh job history without raw payloads, credentials, or path values. |
| `refresh_provider_import_profile` | draft-only | Validate one configured provider profile and persist a metadata-only refresh job summary. |
