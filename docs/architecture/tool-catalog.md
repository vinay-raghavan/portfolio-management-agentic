...
| `refresh_provider_import_profile` | draft-only | Validate one configured provider profile, persist a sanitized refresh job summary, and import configured market snapshots into the structured market-data store when configured. |
...
The product data, provider, pattern, recommendation, report, strategy-history, backtest, and paper-ledger tools do not write live orders, retrieve broker trading tokens, expose provider credential values, or bypass paper-trading approval gates. Read-only provider tools may read locally configured JSON snapshot, universe, fundamentals, sentiment, volatility, and macro files when `PORTFOLIO_MARKET_DATA_PROVIDER=json_file` / `PORTFOLIO_MARKET_DATA_JSON_PATH`, `PORTFOLIO_UNIVERSE_PROVIDER=json_file` / `PORTFOLIO_UNIVERSE_JSON_PATH`, `PORTFOLIO_FUNDAMENTALS_PROVIDER=json_file` / `PORTFOLIO_FUNDAMENTALS_JSON_PATH`, `PORTFOLIO_SENTIMENT_PROVIDER=json_file` / `PORTFOLIO_SENTIMENT_JSON_PATH`, `PORTFOLIO_VOLATILITY_PROVIDER=json_file` / `PORTFOLIO_VOLATILITY_JSON_PATH`, and `PORTFOLIO_MACRO_PROVIDER=json_file` / `PORTFOLIO_MACRO_JSON_PATH` are set, and may persist local cache rows when `MARKET_DATA_DB_PATH` is configured. Import validation uses the same provider parsers, reports only status and sample identifiers, and does not return local paths or provider data files. Provider profile refresh jobs are draft-only writes when `PROVIDER_CONFIG_DB_PATH` is configured; they store validation status, counts, sample identifiers, env key names, source labels, and sanitized execution metadata, not resolved paths or raw provider payloads. Configured market-data refreshes may additionally write normalized snapshots to `market_data_snapshots`; credential-looking metric keys and raw payload notes are not retained by that import path. Cache writes are limited to market snapshots, screener-run payloads, paper-ledger state, and provider metadata. Simulated fills are paper-only, require prior approval, and update only the paper ledger. Report tools return JSON-ready data and do not write files.
