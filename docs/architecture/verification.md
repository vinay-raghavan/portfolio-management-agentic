...
- Web console contracts verify the Vite/React app, safe product surfaces, Compose wiring, `/console/overview`, focused `/console/workflows` pages, provider source setup visibility, required env-key display, active adapter modes, setup-gap feedback, provider import validation feedback, provider profile/import-job settings, full provider refresh controls, per-provider backoff state, and the paper-only action lifecycle.
