...
- Agent definition contracts verify the ADK instruction contains eval-aligned workflow routes and refusal guidance, and that runtime callbacks short-circuit forbidden/human-API routes, filter paper-proposal tools to the capability manifest, and block out-of-route stale tool calls before model-backed evals run.
...
- Credential-vault readiness contracts fail closed by default, require explicit
  ...
  Postgres while redacting the ref value from HTTP payloads.
